...
void KZDatabaseService::SetupDatabase()
{
	...
	if (!config)
	{
		KZ_LOG_INFO(LogChannel::General, "[KZ::DB] No database config detected.\n");
		return;
	}
	ISQLInterface *sqlInterface = (ISQLInterface *)g_SMAPI->MetaFactory(SQLMM_INTERFACE, nullptr, nullptr);
	if (!sqlInterface)
	{
		KZ_LOG_INFO(LogChannel::General, "[KZ::DB] Database plugin not found. Local database is disabled.\n");
		return;
	}
...
void KZDatabaseService::OnDatabaseConnected(bool connect)
{
	...
	{
		KZ_LOG_WARN(LogChannel::DB, "Failed to connect\n");
