...
void KZRacingService::ItemDownloadHandler::OnAddonDownloaded(DownloadItemResult_t *pParam)
{
	...
	{
		KZ_LOG_INFO(LogService::General, "[KZ::Racing] Failed to download workshop item %llu, error code: %d\n", pParam->m_nPublishedFileId, pParam->m_eResult);
		return;
	}

	KZ_LOG_INFO(LogService::General, "[KZ::Racing] Successfully downloaded workshop item %llu\n", pParam->m_nPublishedFileId);
