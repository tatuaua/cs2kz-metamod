...
void KZTimerService::InsertRecordToCache(f64 time, const KZCourseDescriptor *course, PluginId modeID, bool overall, bool global, CUtlString metadata)
{
	...
	if (!error.IsEmpty())
	{
		KZ_LOG_INFO(LogChannel::Timer, "Failed to insert PB to cache due to metadata error: %s\n", error.Get());
		return;
	}
									 ...
									 f64 points)
{
	...
	if (!error.IsEmpty())
	{
		KZ_LOG_INFO(LogChannel::Timer, "Failed to insert server record to cache due to metadata error: %s\n", error.Get());
		return;
	}
...
void KZTimerService::ShowSplitText(u32 currentSplit)
{
	...
	if (pb)
	{
		...
		{
			KZ_LOG_INFO(LogChannel::Timer, "pb->overall.pbSplitZoneTimes[currentSplit - 1] = %lf\n", pb->overall.pbSplitZoneTimes[currentSplit - 1]);
...
CUtlString KZTimerService::GetCurrentRunMetadata()
{
	...
	KZ_LOG_INFO(LogChannel::Timer, "Failed to obtain current run's metadata! (%s)\n", error.Get());
	return "";
}
