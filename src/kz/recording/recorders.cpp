...
bool Recorder::WriteToFile()
{
	...
	if (!file)
	{
		KZ_LOG_INFO(LogService::General, "Failed to open replay file for writing: %s\n", tempFilename);
		return false;
	}
	...
	{
		KZ_LOG_INFO(LogService::General, "kz_replay_recording_debug: Wrote replay header (%d bytes)\n", bytesWritten);
	}
	bytesWritten += KZ::replaysystem::compression::WriteTickDataCompressed(file, this->tickData, this->subtickData);

	if (kz_replay_recording_debug.Get())
	{
		KZ_LOG_INFO(LogService::General, "kz_replay_recording_debug: Wrote tick data (%d bytes)\n", bytesWritten);
	}
	bytesWritten += KZ::replaysystem::compression::WriteWeaponsCompressed(file, this->weaponTable);

	if (kz_replay_recording_debug.Get())
	{
		KZ_LOG_INFO(LogService::General, "kz_replay_recording_debug: Wrote weapons (%d bytes)\n", bytesWritten);
	}
	bytesWritten += KZ::replaysystem::compression::WriteJumpsCompressed(file, this->jumps);

	if (kz_replay_recording_debug.Get())
	{
		KZ_LOG_INFO(LogService::General, "kz_replay_recording_debug: Wrote jumps (%d bytes)\n", bytesWritten);
	}
	bytesWritten += KZ::replaysystem::compression::WriteEventsCompressed(file, this->rpEvents);

	if (kz_replay_recording_debug.Get())
	{
		KZ_LOG_INFO(LogService::General, "kz_replay_recording_debug: Wrote events (%d bytes)\n", bytesWritten);
	}
	bytesWritten += KZ::replaysystem::compression::WriteCmdDataCompressed(file, this->cmdData, this->cmdSubtickData);

	if (kz_replay_recording_debug.Get())
	{
		KZ_LOG_INFO(LogService::General, "kz_replay_recording_debug: Wrote cmd data (%d bytes)\n", bytesWritten);
	}
	...
	{
		KZ_LOG_INFO(LogService::General, "Failed to rename replay file from %s to %s\n", tempFilename, finalFilename);
	...
	if (kz_replay_recording_debug.Get())
	{
		KZ_LOG_INFO(LogService::General, "kz_replay_recording_debug: Saved replay to %s (%d bytes)\n", finalFilename, bytesWritten);
	}
...
i32 Recorder::WriteHeader(FileHandle_t file)
{
	...
	{
		KZ_LOG_INFO(LogService::General, "[KZ] Failed to serialize replay header protobuf\n");
		return 0;
	}
