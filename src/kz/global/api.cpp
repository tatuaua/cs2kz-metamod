...
bool KZ::api::PlayerInfo::FromJson(const Json &json)
{
	if (!json.Get("id", this->id))
	{
		...
		{
			KZ_LOG_INFO(LogService::General, "[KZ::Global] Failed to parse SteamID2.\n");
			return false;
		}
	}
...
bool KZ::api::Map::DecodeStateString(std::string_view stateString, State &state)
{
	...
	{
		KZ_LOG_INFO(LogService::General, "[KZ::Global] `state` field has an unknown value.\n");
		return false;
	}
...
bool KZ::api::Map::Course::Filter::DecodeTierString(std::string_view tierString, Tier &tier)
{
	...
	{
		KZ_LOG_INFO(LogService::General, "[KZ::Global] `tier` field has an unknown tierString.\n");
		return false;
	}
