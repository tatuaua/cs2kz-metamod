...
private:
	...
	public:
		...
		{
			KZ_LOG_WARN(LogChannel::Global, "Received error response to WebSocket message (id=%i): %s\n", messageID, error.message.c_str());
		}
...
public:
	...
	inline static bool MayBecomeAvailable()
	{
		return state.load() != State::Disconnected;
	}
