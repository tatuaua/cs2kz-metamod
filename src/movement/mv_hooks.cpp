	...
	if (traceHistory.Count() != count)
	{
		for (i32 i = 0; i + count < traceHistory.Count(); i++)
		{
			if (traceHistory[i].end != traceHistory[i + count].end)
			{
				...
				for (i32 j = 0; j <= i; j++)
				{
					KZ_LOG_INFO(LogChannel::General, "Pred %f %f %f -> %f %f %f, error %f, velocity %s ", traceHistory[j].start.x, traceHistory[j].start.y,
					...
					KZ_LOG_INFO(LogChannel::General, "Real %f %f %f -> %f %f %f, error %f, velocity %s ", traceHistory[j + count].start.x, traceHistory[j + count].start.y,
