											 ...
											 bool *bIsSurfing)
{
	...
	if (traceHistory.Count() != count)
	{
		for (i32 i = 0; i + count < traceHistory.Count(); i++)
		{
			...
			{
				KZ_LOG_INFO(LogService::General, "Trace not matching! Previous traces (initial error %f, initial velocity %s):\n", initialError, VecToString(initialVelocity));
				for (i32 j = 0; j <= i; j++)
				{
					KZ_LOG_INFO(LogService::General, "Pred %f %f %f -> %f %f %f, error %f, velocity %s ", traceHistory[j].start.x, traceHistory[j].start.y,
					...
					KZ_LOG_INFO(LogService::General, "Real %f %f %f -> %f %f %f, error %f, velocity %s ", traceHistory[j + count].start.x, traceHistory[j + count].start.y,
