...
namespace HTTP
{
	...
	void Request::Send(ResponseCallback onResponse) const
	{
		if (!g_pHTTP)
		{
			KZ_LOG_INFO(LogService::General, "[HTTP] Initializing HTTP client...\n");
			if (g_steamAPI.Init())
			{
			...
			{
				KZ_LOG_INFO(LogService::General, "[HTTP] Failed to send HTTP request as the steam API is not yet initialized.\n");
				return;
			}
		}
		...
		if (method >= Method::POST)
		{
			...
			{
				KZ_LOG_INFO(LogService::General, "[HTTP] Failed to set request body.\n");
				return;
			}
		}
		...
		{
			KZ_LOG_INFO(LogService::General, "[HTTP] Failed to send HTTP request.\n");
		}
	...
	void InFlightRequest::OnRequestCompleted(HTTPRequestCompleted_t *completedRequest, bool failed)
	{
		if (failed)
		{
			KZ_LOG_INFO(LogService::General, "[HTTP] request to `%s` failed with code %d\n", url.c_str(), completedRequest->m_eStatusCode);
			delete this;
			return;
		}
